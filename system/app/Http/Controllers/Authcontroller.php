...
class AuthController extends Controller
{
    ...
    function Loginprocess(){
        ...
        if(Auth()->attempt(['email' => request('email'),'password' => request('password')])){
            $user = Auth()->user();
            if($user->level == 1) return redirect('dashboard')->with('succes', 'Login Berhasil');
...
function showClientLogin(){
    return view('frontview.clientlogin');
}

function ClientLoginprocess(){
    // if(Auth()->attempt(['email' => request('email'), 'password' => request('password')])){
    ...
    $user = Penjual::where('email', $email)->first();
        ...
        if(!$guard) {
            ...
            if(Auth()->guard($guard)->attempt(['email' => request('email'), 'password' => request('password')])){
                return redirect("frontview.index")->with('success', 'Login Berhasil');
            }else{
                return back()->with('danger', 'Login Gagal,Silahkan cek email dan password anda');
            }
        }
...
function ClientLogout(){
    Auth()->logout();
    return redirect('frontview.index');
}
}